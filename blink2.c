...
static int usb_read_serial(void) {
    ...
    while(1) {
        ...
        if(ret) {
            if(ret == -ETIMEDOUT) {
                printk(KERN_ERR "SmartLamp: Timeout na leitura USB (retorno %d)\n", ret);
            } else {
                printk(KERN_ERR "SmartLamp: Erro na leitura USB (retorno %d)\n", ret);
            }
            return -1;
        }
        ...
        for (i = 0; i < actual_size; i++) {
            if (recv_size < MAX_RECV_LINE -1 ){
                ...
                if (usb_in_buffer[i] == '\n') {
                    ...
                    printk(KERN_INFO "SmartLamp: Resposta completa recebida: %s", recv_line);

                    if(sscanf(recv_line, "RES GET_LDR %d", &valor_ldr) == 1) {
                        return valor_ldr;
                    } else {
                        printk(KERN_ERR "SmartLamp: Erro ao extrair valor com sscanf da linha: %s", recv_line);
                        return -1;
                    }
                }
            } else {
